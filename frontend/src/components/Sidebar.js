 ...
 *
 * File cards are displayed in a 2-column grid so each card can be
 * near-square with a prominent coloured thumbnail.
 *
 * Two drag interactions are supported:
 *  1. OS → Upload zone   drag PDFs from the file system to upload them
 *  2. Workspace → Sidebar   drag an open workspace panel back here to close it
 *
 * Props
 *  draggedWorkspaceFile  file object being dragged from the workspace (or null)
 *  onDropFromWorkspace   called with the file when dropped back onto the sidebar
 */
export default function Sidebar({
  ...
  const handleRestoreDragOver = (e) => {
    if (!draggedWorkspaceFile) return;
    e.preventDefault();
  ...
  return (
    ...
    >
      {/* ── Logo / header ── */}
      <div className="px-5 pt-5 pb-4 border-b border-apple-border flex-shrink-0">
      ...
      {/* ── Restore-to-library indicator ─────────────────────────────────────
          Shown while a workspace panel is being dragged, guiding the user to
          drop here in order to close the panel. */}
      {draggedWorkspaceFile && (
        <div className={`mx-4 mt-3 rounded-2xl border-2 border-dashed py-2.5 text-center
      ...
      <div className="px-4 pt-3 pb-2 flex-shrink-0">
        ...
        >
          <input {...getInputProps()} />
          <div className={`w-8 h-8 rounded-xl mx-auto mb-2 flex items-center justify-center transition-colors
            ${isDragActive ? "bg-apple-blue" : "bg-gray-100 group-hover:bg-apple-blue"}`}>
            <Upload size={15} className={`transition-colors
      ...
      {/* ── 2-column card grid ───────────────────────────────────────────────
          Each FileItem is a near-square card with a coloured thumbnail.
          Two columns fit comfortably in the 288 px sidebar. */}
      ...
      {loading && (
        <div className="px-5 py-3 border-t border-apple-border flex items-center gap-2 flex-shrink-0">
